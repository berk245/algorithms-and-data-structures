...
'''
        ...
        While these two values are not equal:
            ...
            4) If they are not equal and
                a. target value is smaller than the array[midpoint]
                    ...
                    - Since the array is sorted, we know that if the midpoint is greater than the target, every value that is located after the 
                ...
                b. target value is greater than the array[midpoint]
                    ...
                    - Since the array is sorted, we know that if the midpoint is smaller than the target, every value that is located before the 
    ...
    TIME COMPLEXITY: O(log n)
        ...
        - In each iteration of the while loop, the size of the array is halved. That is why when the array size(n) is doubled, the algorithm needs only one more step to complete the search. 
        -Below you can see the required steps for binary search to complete with different array sizes.
            Target: Last element of array
    ...
    SPACE COMPLEXITY: O(1)
        ...
        Since there is no need for storing additional data during the execution of the algorithm, the space complexity is constant and independent of the array size.
    ...
    USE CASES
    Logarithmic runtime is more efficient compared to linear runtime, therefore the binary search is a preferred searching algorithm. 
    However, if the array is not sorted, binary search does not return correct results. 
    Depending on the case (size of the array, position of the target), using another search algorithm, such as linear search, might be more time-efficient than sorting the array and executing the search afterward.
'''
